...
Changes to `ALLOW_SIGNUP`, `DISABLE_WORKSPACE_CREATION`, `GOOGLE_CLIENT_ID`, and the `OIDC_*` single sign-on variables also take effect after restarting the backend / compose stack. The web UI reads them all from `/api/config` at runtime, so no web rebuild is needed. See [Advanced Configuration → Signup Controls](SELF_HOSTING_ADVANCED.md#signup-controls-optional) for the recommended sequence to lock down workspace creation.
...
`ALLOW_SIGNUP`, `DISABLE_WORKSPACE_CREATION`, `GOOGLE_CLIENT_ID`, and single sign-on likewise live under `backend.config.*` in `values.yaml` (as `allowSignup`, `disableWorkspaceCreation`, `googleClientId`, and the `oidc.*` block; `OIDC_CLIENT_SECRET` goes in the pre-created Secret instead). After `helm upgrade`, the backend pod will roll automatically because the ConfigMap hash changes; the web UI reads them all from `/api/config` at runtime, so no web rebuild is needed.
