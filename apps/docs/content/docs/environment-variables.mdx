...
| Variable | Default | Description |
|---|---|---|
| `OIDC_ISSUER` | empty | Issuer URL serving `/.well-known/openid-configuration`. Empty keeps SSO off |
| `OIDC_CLIENT_ID` | empty | Client ID registered with the provider |
| `OIDC_CLIENT_SECRET` | empty | Client secret. Required — the flow is confidential-client |
| `OIDC_PROVIDER_NAME` | `SSO` | Label on the sign-in button |
| `OIDC_SCOPES` | `openid email profile` | Comma- or space-separated; `openid` is added if omitted |
| `OIDC_REDIRECT_URI` | derived from `MULTICA_PUBLIC_URL`, else `MULTICA_APP_URL` | Callback URL registered with the provider, i.e. `<origin>/auth/oidc/callback` |
| `RATE_LIMIT_AUTH_SSO` | `60` | SSO redirects allowed per IP per minute |
